...
final class MathFunc {
	...
	static double Max(Stack<Double> list) {
		if (list.empty())
	...
	static double Min(Stack<Double> list) {
		if (list.empty())
	...
	static double Sum(Stack<Double> list) {
		if (list.empty())
	...
	static double Mean(Stack<Double> list) {
		if (list.empty())
	...
	static double Round(Stack<Double> list) {
		if (list.size() > 2)
...
/* XXX TODO Allow Bitwise Operators, taking only integer part; Useful in Mode-4 */
		...
		if (polyOperators.contains(op)) {
			...
			if (op.equals("sum") || op.equals("avg") || op.equals("mean")) {
				...
				if (op.equals("ncr") || op.equals("npr") || op.equals("pow")) {
					ans = (op.equals("ncr")) ? Func.nCr(x, y) : (op.equals("npr")) ? Func.nPr(x, y) : Math.pow(x, y);
				}
			}
...
//					throw new BadNumberException("<Bitwise Operands!>\nIntegers, Not Decimals");
			...
			if (op.equals("u+") || op.equals("u-")) {
				...
				ans = (op.equals("sqrt")) ? Math.sqrt(x) : (op.equals("cbrt")) ? Math.cbrt(x) : Func.fact(x);
...
//				System.out.println(soln);
	...
	public static void main(String[] args) {
		double ans = solveScientific("+5+-+-3");
